...
THREADS = 10
PTR_FOLLOW_OTHER = False  # if ptrs pointing to other parent-domains should be scanned (1-layer deep)
...
def main():
    ...
    def _check_ptrs(ptrs: dict, ptr_domain: str):
        ...
        for ipp in ['ip4', 'ip6']:
            for d in ptrs[ipp]:
                d = d[:-1]
                if d != ptr_domain and (PTR_FOLLOW_OTHER or d.find(TARGET) != -1) and d not in results:
    ...
    if wildcard_exists:
        ...
        results[f'*.{TARGET}'] = {'ip': wildcard_ips, 'ptr': wildcard_ptrs}
        _check_ptrs(wildcard_ptrs, f'*.{TARGET}')
    ...
    try:
        ...
        while batch * THREADS < len(wordlist):
            ...
            for i in range(THREADS):
                threads.append(Thread(target=_lookup, kwargs={'word': wordlist[(batch * THREADS) + i]}))
